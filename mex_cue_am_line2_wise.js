...
var TableSuplierct = null,
  ...
  TableSupliertimeStop = 70, //NOTE: Timestop
...
var Coderct = null,
  ...
  CoderONS = false,
  CodertimeStop = 70, //NOTE: Timestop
...
var Monoblockct = null,
  ...
  MonoblockONS = false,
  MonoblocktimeStop = 90, //NOTE: Timestop
  ...
  MonoblockVerify = (function() {
    try {
      ...
      if (err.code == 'ENOENT' || err == 12121212) {
        fs.writeFileSync('MonoblockRejected.json', '{"rejected":0}') //NOTE: Change the object to what it usually is.
        MonoblockReject = {
          rejected: 0
        }
      }
...
var GasFillerct = null,
  ...
  GasFillerONS = false,
  GasFillertimeStop = 90, //NOTE: Timestop
...
var TestBathct = null,
  ...
  TestBathONS = false,
  TestBathtimeStop = 90, //NOTE: Timestop
...
var Capperct = null,
  ...
  CapperONS = false,
  CappertimeStop = 90, //NOTE: Timestop
...
var Bundlerct = null,
  ...
  BundlerONS = false,
  BundlertimeStop = 90, //NOTE: Timestop
...
var Dividerct = null,
  ...
  DividerONS = false,
  DividertimeStop = 90, //NOTE: Timestop
...
var CaseFormerct = null,
  ...
  CaseFormerONS = false,
  CaseFormertimeStop = 90, //NOTE: Timestop
...
var CasePackerct = null,
  ...
  CasePackerONS = false,
  CasePackertimeStop = 80, //NOTE: Timestop
...
var CaseSealerct = null,
  ...
  CaseSealerONS = false,
  CaseSealertimeStop = 90, //NOTE: Timestop
...
var CheckWeigherct = null,
  ...
  CheckWeighertimeStop = 90, //NOTE: Timestop
...
client2.on('connect', function(err) {
  ...
  cA2 = setInterval(function() {
    client2.readHoldingRegisters(0, 16).then(function(resp) {
      ...
      CntInMachineTestFirst = CntInMonoblock;
      CntOutMachineTestFirst = CntOutMonoblock;
    });
    //------------------------------------------Monoblock----------------------------------------------
    ...
    if (GasFillerflagPrint == 1) {
      for (var key in GasFillerresults) {
        if (GasFillerresults[key] != null && !isNaN(GasFillerresults[key])) {
          ...
          //fs.appendFileSync('C:/PULSE/AM_L2/L2_FAKE/mex_cue_GasFiller_l2.log', 'tt=' + GasFillertime + ',var=' + key + ',val=' + GasFillerresults[key] + '\n')
        }
      }
      GasFillerflagPrint = 0
      GasFillersecStop = 0
...
client3.on('connect', function(err) {
  ...
  cA3 = setInterval(function() {
    client3.readHoldingRegisters(0, 16).then(function(resp) {
      ...
      CntOutMachineTestSecond = CntOutXray;
      CntInMachineTestSecond = CntInXray;
    });
    //------------------------------------------Xray----------------------------------------------
...
function getRejects() {
  ...
  fs.writeFileSync('MonoblockRejected.json', '{"rejected": ' + MonoblockReject.rejected + '}')
  var GasFillerDif = CntInGasFiller - CntOutGasFiller
  fs.appendFileSync('C:/PULSE/AM_L2/L2_LOGS/mex_cue_GasFiller_l2.log', 'tt=' + Date.now() + ',var=CPQR,val=' + eval(GasFillerDif - GasFillerReject.rejected) + '\n')
  GasFillerReject.rejected = GasFillerDif
  fs.writeFileSync('GasFillerRejected.json', '{"rejected": ' + GasFillerReject.rejected + '}')
  var XrayDif = CntInXray - CntOutXray
  fs.appendFileSync('C:/PULSE/AM_L2/L2_LOGS/mex_cue_Xray_l2.log', 'tt=' + Date.now() + ',var=CPQR,val=' + eval(XrayDif - XrayReject.rejected) + '\n')
  XrayReject.rejected = XrayDif
  fs.writeFileSync('XrayRejected.json', '{"rejected": ' + XrayReject.rejected + '}')
  var TestBathDif = CntInTestBath - CntOutTestBath
  fs.appendFileSync('C:/PULSE/AM_L2/L2_LOGS/mex_cue_TestBath_l2.log', 'tt=' + Date.now() + ',var=CPQR,val=' + eval(TestBathDif - TestBathReject.rejected) + '\n')
  TestBathReject.rejected = TestBathDif
  fs.writeFileSync('TestBathRejected.json', '{"rejected": ' + TestBathReject.rejected + '}')
  var CheckWeigherDif = CntInCaseSealer - CntOutCheckWeigher
